...
class MoleculeTest extends AnyWordSpec with Matchers {
  ...
  val in_mol1 = "src/test/resources/Input/input1.vel"
  val nombre = "water"
  ...
  private val c_mass1 = List(BigDecimal("0E-13").setScale(13), BigDecimal("-0.0000173119209").setScale(13), BigDecimal("0E-13").setScale(13))
  ...
  it should {
    ...
    "obtain the atomic number of the atoms" in {
      val c_ch = r_mol.c_at_f.orderBy("AtomID").collect().map(_.getAs[Int]("Num_at")).toList
      c_ch shouldBe c_ch1
    }
    "obtain the mass of the atoms" in {
      val masa = r_mol.c_at_f.orderBy("AtomID").collect().map(_.getAs[Double]("Mass")).toList
      masa shouldBe mass1
    }
    "obtain the center of mass" in {
      val cast_cm = r_mol.mol_f.withColumn(Par.c_cm, col(Par.c_cm).cast("array<decimal(25,13)>"))
      val c_mass = cast_cm.collect().map(_.getAs[Seq[BigDecimal]]("Center_mass")).apply(0).toList
      c_mass shouldBe c_mass1_j
    }
  }

  val in_mol2 = "src/test/resources/Input/input2.vel"
  val nombre2 = "test_at"
  val r_mol1 = new Molecule(in_mol2)

  //Lista de valores del centro de masas
  private val c_mass2 = List(BigDecimal("0.24216318999483818").setScale(13, RoundingMode.HALF_EVEN),
  ...
  it should {
    "obtain the center of mass" in {
      ...
      val cast_cm = r_mol1.mol_f.withColumn(Par.c_cm, col(Par.c_cm).cast("array<decimal(25,13)>"))
