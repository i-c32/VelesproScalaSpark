...
import VelESPro.App.{logger, spark}
...
class Molecule(inOpFin : String) {
  ...
  logger.info("Se lee el fichero de configuracion")
  ...
  logger.info("Se crea el dataframe de la molecula")
  private val molIni = liMolec.map(mol => Molec(
    mol,
    configMol.getString(s"$mol.method"),
    configMol.getString(s"$mol.basis set"),
    configMol.getInt(s"$mol.charge"),
    configMol.getInt(s"$mol.multiplicity"))
  ).toList.toDS()

  // Se lee la basis set
  logger.info("Se carga el basis set")
  ...
  logger.info("Se cargan las coordenadas")
  ...
  private val coords = Seq(Par.cCx, Par.cCy, Par.cCz)

  private val cAt1 = coords.foldLeft(cIniB) { (df, colName) =>
    df.withColumn(
      colName,
      when(col("bbohr") === false, col(colName) / Par.cBohr)
  ...
  logger.info("Se calculan las distancias")
  ...
  private val dfMDist1 = dfMDist.withColumn("Dist_at",array(nameCols.map(col): _*)).select("AtomID", "Dist_at")
  val cAtF: DataFrame = cAt4.join(dfMDist1,Seq("AtomID"))

  // Se añade la masa y el centro de masas al dataframe de la molecula
  logger.info("Se calcula el centro de masas")
  ...
  logger.info("Se calcula la repulsion electrica")
